...
export default function AddReportForSchoolScreen() {
  const { schoolId } = useLocalSearchParams<{ schoolId: string }>();
  const router = useRouter();
  const { getSchoolById, addReport } = useSchoolStore();
  ...
  // Get current month and year for period
  ...
  const [officeApplications, setOfficeApplications] = useState(false);
  ...
  const handleSubmit = () => {
    ...
    Alert.alert("Report Added", "ICT report has been successfully added.", [
      {
        text: "OK",
        onPress: () => router.replace(`/schools/${schoolId}`),
      },
    ]);
  };
          ...
          <View style={styles.section}>
            ...
            <Button
              ...
              onPress={() => {
                ...
                Alert.alert(
                  "Photo Upload",
                  "This would open the camera or photo gallery in a real app."
                );
              }}
              variant="outline"
